<template>
  <div>
    <!-- Page Header -->
    <div class="mb-8">
      <h1 class="mb-2 text-3xl font-bold text-gray-900">Billing & Shipping</h1>
    ...
    <form class="space-y-6 wn-form" @submit.prevent="saveChanges">
      ...
      <div class="bg-white border border-gray-100 rounded-lg shadow-xs">
        <div class="p-6 pb-4 border-b border-gray-100 md:px-8">
        ...
        <div v-if="customer.billing" class="grid gap-6 p-6 md:p-8 md:grid-cols-2">
          ...
          <div class="w-full space-y-2">
            <label for="billing-address-2" class="block text-sm font-medium text-gray-700"
              >{{ $t('billing.address2') }} <span class="font-normal text-gray-400">({{ $t('general.optional') }})</span></label
            >
            <input
      ...
      <div class="bg-white border border-gray-100 rounded-lg shadow-xs">
        ...
        <!-- Submit Button -->
        <div class="p-6 pt-4 border-t flex border-gray-100 rounded-b-lg bg-gray-50">
