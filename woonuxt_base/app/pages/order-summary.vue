...
<template>
  <div
    class="order-receipt-container w-full min-h-150 flex items-center p-4 text-gray-800 md:bg-white md:rounded-xl md:mx-auto md:shadow-lg md:my-24 md:mt-8 md:max-w-3xl md:p-16 flex-col">
        ...
        <template v-if="isSummaryPage">
          <div class="flex items-center gap-4">
            <NuxtLink
              ...
              class="inline-flex items-center justify-center p-2 border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
              title="Back to orders"
              aria-label="Back to orders">
              <Icon name="ion:chevron-back-outline" />
            </NuxtLink>
            <h1 class="text-xl font-semibold text-gray-900">{{ $t('shop.orderSummary') }}</h1>
          </div>
        </template>
        <template v-else-if="isCheckoutPage">
          <div class="flex items-center justify-between w-full mb-2">
            <h1 class="text-xl font-semibold text-gray-900">{{ $t('shop.orderReceived') }}</h1>
            <button
              v-if="orderIsNotCompleted"
              type="button"
              class="inline-flex items-center justify-center p-2 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors"
              title="Refresh order"
          ...
          <p class="text-gray-600">{{ $t('shop.orderThanks') }}</p>
        </template>
      ...
      <div v-if="order" class="flex-1 w-full">
        <div class="flex items-start justify-between">
          <div class="w-[21%]">
            ...
            <div class="leading-none text-gray-900">#{{ order.databaseId! }}</div>
          </div>
          <div class="w-[21%]">
            <div class="mb-2 text-xs text-gray-400 uppercase">{{ $t('general.date') }}</div>
            <div class="leading-none text-gray-900">{{ formatDate(order.date) }}</div>
          </div>
          <div class="w-[21%]">
          ...
          <div class="w-[21%]">
            ...
            <div class="leading-none text-gray-900">{{ order.paymentMethodTitle }}</div>
          </div>
        </div>
